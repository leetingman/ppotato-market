			...
			$.ajax({
				...
				success: function(result) {
					...
					$("#buttonDiv" + result.commentId).on("click", result.commentId, function() {
						...
						if (!confirm("Are you sure you want to delete your comments? You can't undo this")) {
							return false;
						} else {
							//ajax 전송시, 'x-csrf-token' 같은 헤더 정보를 추가해서 csrf 토큰값 전달
							var csrfHeaderName = "${_csrf.headerName}";
...
$(function() {
	$(".deleteCommentAtag").click(function() {
		...
		if (!confirm("Are you sure you want to delete your Comment? You can't undo this")) {
			return false;
		} else {
			//ajax 전송시, 'x-csrf-token' 같은 헤더 정보를 추가해서 csrf 토큰값 전달
			var csrfHeaderName = "${_csrf.headerName}";
