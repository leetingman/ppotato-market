	...
	$(document).ready(
		function() {
			$.ajax({
				...
				$("#IdCheckResult").text("Letters, numbers, dashes, and underscores only. Please try again without symbols").css("color", "red");
				}else if(result == 0){
					$("#IdCheckResult").text("This ID is available").css("color", "green");
					id = 0;
					}else{
					$("#IdCheckResult").text("That username is already taken").css("color", "red");
					id = 1;
					}
				...
				//	if (id_rule.test(IdCheckInfo)) {
				//	$("#IdCheckResult").text("Letters, numbers, dashes, and underscores only.").css("color", "red");
				//}
					...
					//	$("#IdCheckResult").text("Letters, numbers, dashes, and underscores only.").css("color", "red");
					//} else if(result == 0) {
					//	$("#IdCheckResult").text("The ID is available").css("color", "green");
					//	id = 0;
		...
		function() {
			$.ajax({
				...
				success: function(result) {
					//span요소인 IdCheckResult 에 뿌려준다
					 if(result == 0) {
						$("#telCheckResult").text("This Phone is available").css("color", "green");
						TelCheck=0;
					}else{
						$("#telCheckResult").text("That Phone is already taken").css("color", "red");
						TelCheck=1;
					}
				}//alert(result);
			}); //$.ajax
...
function passwordChek1(){
	...
	if(!password_rule.test(pass1)){
		...
		document.getElementById("passwordResult1").innerHTML = "Letters, numbers, dashes, and underscores only. Please try again without symbols";
	}else{
		document.getElementById("passwordResult1").style.color = "green";
		document.getElementById("passwordResult1").innerHTML = "Available.";
	}
}
function passwordChek2() {
	...
	if (pass1 != pass2) {
		...
		document.getElementById("passwordResult2").innerHTML = "Your password is incorrect.";
	} else {
		document.getElementById("passwordResult2").style.color = "green";
		document.getElementById("passwordResult2").innerHTML = "Your password is correct";
	}
}
...
$(document).ready(function() {
	$("#registerForm").submit(function() {
		...
		if (id == 1) {
			...
			var special_pattern = /[`~!@#$%^&*|\\\'\";:\/?]/gi;
			if( special_pattern.test(IdCheckInfo) == true ){
			alert("Letters, numbers, dashes, and underscores only. Please try again without symbols");
			$("#email_id").val("");
			return false;
			}
