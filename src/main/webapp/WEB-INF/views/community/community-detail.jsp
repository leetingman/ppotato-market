                     ...
                     <c:choose>
                     	<c:when test="${userId==communityData.userVO.userId}">
                    		<div class="float-right float-xs-none d-xs-none">
                       	 		<a href="updateCommunityForm?communityId=${communityData.communityId}">Edit</a>&nbsp
                        		<a onclick="return deleteCommunity()" id="deleteAtag">Del</a>
                     		</div>
                     	</c:when>
                     </c:choose>
                  </div>
                           ...
                           <div class="row ">
                              <div class="col-10">
                                 <div class="form-group" class="input_reply_div">
                                    ...
                                    <input name=communityContent type="text" class="form-control" id="commentContent" placeholder="What are you thoughts"  autofocus="autofocus">
                                 </div>
                                 <!-- end form-group -->
                              </div>
                              <div class="col-2">
                                 <div class="form-group">
                                    <button class="btn btn-primary" id="writeCommentBtn" type="button">comment</button>
                                 </div>
                                 <!-- end form-group -->
                              </div>
                           </div>
                                          ...
                                          <div class="meta">
                                             ...
                                             <c:choose>
                     							 <c:when test="${userId==commemtList.userVO.userId}">
                                             		<div class="float-right float-xs-none d-xs-none">
                                                		<a class='updateCommentAtag' data-id=${commemtList.commentId } >Edit</a>&nbsp
                                                		<a id="deleteCommentAtag" class='deleteCommentAtag' data-id=${commemtList.commentId } >Del</a>
                                            	 	</div>
                                            	 </c:when>
                                             </c:choose>
                                          </div>
			...
			$.ajax({
				...
				success:function(result){
                	...
                	/* 	updateInfo+="			<div class='meta'>";
                		...
                		updateInfo+="			<input type='text' id='updateCommentContent' placeholder='edit here'>";
                		updateInfo+="			<div class='meta' style='margin-top:10px;'>";
                		updateInfo+="				<div class='float-right float-xs-none d-xs-none'>";
                		updateInfo+="					<a class='updateCommentConfirm'>submit</a>&nbsp";
                		updateInfo+="					<a>cancel</a>";
                		updateInfo+="				</div>";
                		updateInfo+="			 </div>";
...
$(function(){      
	$(".updateCommentAtag").click(function() {
		...
		updateInfo+="				<div class='float-right float-xs-none d-xs-none'>";
		updateInfo+="					<a class='updateCommentConfirm'>submit</a>&nbsp";
		updateInfo+="					<a class='updateCommentReset'>cancel</a>";
		updateInfo+="				</div>";
		updateInfo+="			 </div>";
		updateInfo+="		 </div> <!--end author-description-->";
		updateInfo+="	 </div> <!--end author-->";
			...
			$.ajax({
				...
				success:function(result){
            	    ...
            	    commentInfo+="               </h5>";
  	              	commentInfo+="               <span>"+result.commentCreatedAt+"</span>";
    	            commentInfo+="               <div class='float-right float-xs-none d-xs-none' id='buttonDiv"+result.commentId+"'>";
        	       	commentInfo+="                  	<a class='updateCommentAtag"+result.commentId+"' >Edit</a>&nbsp ";
        	        commentInfo+="					<a class='deleteCommentAtag"+result.commentId+"'>Del</a>";
            	    commentInfo+="               </div>";
                	commentInfo+="            </div>";
  	             	commentInfo+="            <p>"+result.commentContent+"</p>";
    	            commentInfo+="         </div>";
